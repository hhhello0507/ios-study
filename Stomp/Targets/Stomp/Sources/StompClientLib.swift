...
public class StompClientLib: NSObject {
    ...
    public func sendJSONForDict(
        ...
        do {
            ...
            //print(theJSONText!)
            var header = [StompCommands.commandHeaderContentType:"application/json;charset=UTF-8"]
            header.merge(header) { (current, _) in current }
            //            let header = [StompCommands.commandSend]
    ...
    public func subscribeToDestination(
        destination: String,
        ...
        let header = [StompCommands.commandHeaderDestination: destination, StompCommands.commandHeaderAck: ack, StompCommands.commandHeaderDestinationId: destination]
        self.sendFrame(command: StompCommands.commandSubscribe, header: header, body: nil)
    }
...
//    }
    ...
    private func checkConnectionHeader(connectionHeaders: [String: String] = [:]) -> Bool{
        !connectionHeaders.isEmpty
    }
